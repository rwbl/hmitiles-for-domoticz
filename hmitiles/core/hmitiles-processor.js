...
export function processDevices(devices) {
	...
	tiles.forEach(async (tileElement) => {
		...
		switch (tileType) {
			...
			// =========================================================================
			// DEVICES
			// List of Domoticz devices with aatributes Name and Data.
			// See hmitiles-devices.js.
			// =========================================================================
			case "devices": {
				const gridContainer = tileElement.querySelector('.hmi-value-grid');
				if (!gridContainer) break;
			...
			// =========================================================================
			// LOGMONITOR
			// Set the height of the log terminal. Default 140px. 
			// See hmitiles.css class .hmi-log-terminal.
			// =========================================================================
			case "logmonitor": {
				// Get the tileelement selector defining the log terminal
				const terminal = tileElement.querySelector('.hmi-log-terminal');
				if (terminal) {
					// Get the log height attribute with defaulr
					const logHeight = tileElement.getAttribute('data-log-height') || '140px';
					// Change these values to set the desired pixel dimensions dynamically
					terminal.style.setProperty('height', logHeight, 'important');
					terminal.style.setProperty('max-height', logHeight, 'important');
				}
				rawValue = 0;
				break;
			}

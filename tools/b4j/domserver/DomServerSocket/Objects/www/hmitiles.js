...
function processDevices(devices) {
    ...
    devices.forEach(device => {
		...
		if (DEBUG) console.log("processDevices idx=", device.idx, "data=", device.Data, "type=", device.Type);
...
// =========================================================================
// SIMPLE AUTOMATIC BACKGROUND TIMER POLL
// =========================================================================

/**
 * Automatically requests data packets from the local B4J simulator server
 * and processes the updates into your existing layout structures.
 */
async function fetchDomoticzData() {
    // Dynamically captures http://127.0.0.1:8080 or your host IP address automatically
    const baseUrl = `${window.location.protocol}//${window.location.host}`;
    const targetUrl = `${baseUrl}/json.htm?type=command&param=getdevices&filter=all`;
    ...
    try {
        ...
        const data = await response.json();
        if (data.status === "OK" && data.result) {
            // Passes the data straight into your core existing dashboard engine
            processDevices(data.result);
        }
    } catch (error) {
        console.error("⚠️ Dashboard sync failure:", error);
    }
}

// Start the timer loop when the browser window finishes initializing
document.addEventListener("DOMContentLoaded", () => {
    // 1. Fetch values immediately on page load to clear empty placeholders
    fetchDomoticzData();
    
    // 2. Poll the server automatically every 3000ms (3 seconds)
    // No page reloads, just smooth, fast background text updates.
    setInterval(fetchDomoticzData, 3000); 
});

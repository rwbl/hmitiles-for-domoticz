...
function processDevices(devices) {
    ...
    devices.forEach(device => {
        ...
        matchingTiles.forEach(tileElement => {
			...
			// ANYOTHER DEVICE
			// =========================================================================
			// Set the badge static text or empty if not found.
			if (!displayStatus) {
				displayStatus = "";
				const badge = tileElement.querySelector('.hmi-badge');
				if (badge) {
					displayStatus = badge.textContent;
				}
			}
...
/**
 * Unified 3-Element Architecture Renderer
 * Safely targets text value outputs while protecting static user-defined layout badges.
 * @function renderStandardTile
 */
function renderStandardTile(tileElement, device, displayStatus) {
    // ELEMENT 1 & 3: The Universal Live Value Display Text Box Target
    const valueEl = tileElement.querySelector('.hmi-value');
    if (valueEl) {
        // Purely Generic: Simply output your clean live data string (e.g. "5290 Watt")
        valueEl.textContent = displayStatus;
    }

    // ELEMENT 2: The Status Header Text Badge Component Box
    const badgeEl = tileElement.querySelector('.hmi-badge') || tileElement.querySelector('.hmi-clickable-badge');
    const alarmType = tileElement.getAttribute('data-alarm-type');
    
    if (badgeEl) {
        // DECLARATIVE ACTION: Only pass badge management to the threshold engine if explicitly requested!
        if (alarmType === "up" || alarmType === "down") {
            const parsedNum = parseFloat(device.Data);
            const numericValueForAlarm = !isNaN(parsedNum) ? parsedNum : 0;
            
            // The alarm engine takes over to process dynamic badge texts and color themes safely
            checkAlarmThresholds(device.idx, numericValueForAlarm);
        } else {
            /* SAFE FALLBACK FOR STANDARD TILES (e.g. Solar Info Dashboard):
             * Since no alarm calculations are requested, the engine NEVER touches your custom text labels!
             * Your original layout string (e.g. "PRODUCTION" or "EXPORT") stays exactly as written in HTML.
             * It simply updates the color state accent directly from your data-alarm attribute. */
            const currentAlarmState = tileElement.getAttribute('data-alarm') || "normal";
            tileElement.setAttribute("data-alarm", currentAlarmState);
        }
    }
}

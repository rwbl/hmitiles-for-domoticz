...
4. **Layout Mapping:** It automatically pairs each device's **Type**, **SubType**, and **SwitchType** against the custom layout templates stored inside your external configuration map file: `/templates/core/hmitiles-roomplan.json`.
...
Adding support for new smart home hardware requires **zero changes to the JavaScript core dashboard code**. All device designs are defined cleanly using simple configurations inside `/templates/core/hmitiles-roomplan.json`. 

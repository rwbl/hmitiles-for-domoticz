# Blueprint Multi-Tile-Trend-Page

This blueprint outlines how to create 3 HMITiles displaying Solar Production value (W) with 24-hour trend and last update timestamp.

## Screenshots

![Multi-Tile-Trend-Page](multi-tile-trend-page.png)
...
├── MultiTileTrendpage.html  	# Domoticz custom page wrapper (created below)
└── multitiletrendpage/
...
Create a new file named `index.html` inside your `multitiletrendpage/` subfolder, open it in any text editor, and paste the following clean structure. 
...
* In Tiles 1,2,3 set the device idx according devices list.
...
<head>
    <meta charset="UTF-8">
    <title>Multi Tile Trend Page</title>
...
## Step 4: Create Page wrapper (`MultiTileTrendPage.html`)
Inside the folder `www/templates` create file `MultiTileTrendPage.html` which calls the `index.html` located in the Folder
`www/templates/multitiletrendpage`.

**Content** `MultiTileTrendpage.html`
```
<script>
  window.location.href = "templates/multitiletrendpage/index.html";
</script>
```
...
3. Goto Tab Custom and select `MultiTileTrendPage'
3. **Watch it Live:** 
...
4. It is also possible to direct load your new custom page directly through your running Domoticz instance web portal:
   `http://YOUR_DOMOTICZ_IP:8080/templates/MultiTileTrendPage.html`
